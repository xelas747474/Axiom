...
function generateHistoryFromPrices(
  ...
  const numTrades = 8 + Math.floor(Math.random() * 5); // 8-12 trades
  ...
  for (let i = 0; i < numTrades; i++) {
    const crypto = validCryptos[Math.floor(Math.random() * validCryptos.length)];
    const prices = priceHistory[crypto.id];
    const maxOpen = Math.max(0, prices.length - 6);
    const openIdx = Math.floor(Math.random() * maxOpen);
  ...
  for (const slot of tradeSlots) {
    ...
    // Tiny spread
    const spreadOpen = realOpenPrice * (0.0003 + Math.random() * 0.0005);
    const spreadClose = realClosePrice * (0.0003 + Math.random() * 0.0005);

    // Determine direction based on price movement tendency
    const priceChange = (realClosePrice - realOpenPrice) / realOpenPrice;
    const direction: TradeDirection = Math.random() < 0.6
    ...
    else if (pnl > 0 && Math.random() < 0.3) closeReason = "trailing_stop";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [btcPrices, ethPrices, solPrices] = await Promise.all([
      fetchHistoricalPrices("bitcoin", 7),
      fetchHistoricalPrices("ethereum", 7),
      fetchHistoricalPrices("solana", 7),
    ]);
    ...
    // Generate realistic history from real prices
    const { trades, curve, finalValue } = generateHistoryFromPrices(priceHistory, config);
