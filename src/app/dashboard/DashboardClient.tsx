...
export default function DashboardClient({ initialData = null }: DashboardClientProps) {
  ...
  const [fetchedAt, setFetchedAt] = useState<number>(initialData ? Date.now() : 0);
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 space-y-12">
        ...
        {user ? (
          <>
            <h1 className="relative text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl">
              Bonjour{" "}
              <span className="bg-gradient-to-r from-[var(--color-accent-blue)] via-[var(--color-accent-purple)] to-[var(--color-accent-cyan)] bg-clip-text text-transparent">
                {user.name.split(" ")[0]}
              </span>
            </h1>
            ...
            {/* User stats row */}
            <div className="relative mt-6 flex items-center justify-center gap-4 flex-wrap animate-fade-in" style={{ animationDelay: "300ms" }}>
              <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
                user.plan === "pro"
                  ? "bg-gradient-to-r from-[var(--color-accent-blue)]/20 to-[var(--color-accent-purple)]/20 text-[var(--color-accent-cyan)] border border-[var(--color-accent-cyan)]/20"
              ...
              {(() => {
                ...
                return (
                  <span className="text-xs text-[var(--color-text-muted)]">
                    Actif depuis <span className="text-white font-medium">{days} jour{days > 1 ? "s" : ""}</span>
                  </span>
                );
              })()}
              {user.role === "admin" && (
                <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-warning)]/10 border border-[var(--color-warning)]/20 px-2.5 py-1 text-[10px] font-bold text-[var(--color-warning)] uppercase tracking-wider">
                  Admin
                </span>
              )}
            </div>
      ...
      <section id="market-overview" className="animate-fade-in-up scroll-mt-24" style={{ animationDelay: "100ms" }}>
        <div className="flex items-end justify-between mb-5 flex-wrap gap-3">
          ...
          <FreshnessIndicator fetchedAt={fetchedAt} isLive={isLive} loading={loading} />
        </div>
      ...
      <section className="animate-fade-in-up" style={{ animationDelay: "200ms" }}>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="premium-card premium-card-positive p-5">
            ...
            <div className="space-y-2 stagger-children">
              {d.topGainers.map((coin) => (
                <div
                  key={coin.symbol}
                  className="flex items-center justify-between rounded-lg bg-black/20 px-4 py-3 transition-all duration-300 hover:bg-black/30 hover:translate-x-1 animate-slide-in-right"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-white tabular-nums font-mono">
          ...
          <div className="premium-card premium-card-negative p-5">
      ...
      <section className="animate-fade-in-up">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger-children">
          {d.quickInsights.map((insight) => (
            <div key={insight.title} className={`premium-card ${insight.status === "positive" ? "premium-card-positive" : insight.status === "negative" ? "premium-card-negative" : ""} p-5 animate-fade-in-up group`}>
